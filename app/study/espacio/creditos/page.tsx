...
import { Coins, Sparkles, TrendingUp, Zap, BookOpen, FileText, CreditCard, MessageSquare, AlertTriangle, RefreshCw } from "lucide-react";
    ...
    return (
      <>
        ...
        <div className={styles.errorState}>
          ...
          <button className={styles.retryButton} onClick={loadCredits} type="button">
      ...
      <div className={styles.tabContent}>
        ...
        <section className={styles.creditsMainCard}>
          <div className={styles.creditsMainHeader}>
            ...
            <div>
              ...
              <div className={styles.creditsMainNumbers}>
                <span className={styles.creditsRemaining}>{credits.remaining}</span>
          ...
          <div className={styles.creditsProgressBar}>
            <div className={styles.creditsProgressFill} style={{ width: `${Math.min(credits.percentageUsed, 100)}%` }} />
          </div>
        ...
        <section className={styles.creditsCostsCard}>
          <h3><Zap size={18} /> Costo Base por Acción</h3>
          <div className={styles.creditsCostsGrid}>
            <div className={styles.creditsCostItem}>
              <BookOpen size={18} />
              <span>Generar Quiz</span>
              <span className={styles.creditsCostValue}>Desde {credits.costs.EXAM_GENERATION}</span>
            </div>
            <div className={styles.creditsCostItem}>
              <FileText size={18} />
              <span>Generar Notas</span>
              <span className={styles.creditsCostValue}>Desde {credits.costs.NOTE_GENERATION}</span>
            </div>
            <div className={styles.creditsCostItem}>
              <CreditCard size={18} />
              <span>Generar Flashcards</span>
              <span className={styles.creditsCostValue}>Desde {credits.costs.FLASHCARD_GENERATION}</span>
            </div>
            <div className={styles.creditsCostItem}>
              <MessageSquare size={18} />
              <span>Chat</span>
              <span className={styles.creditsCostValue}>{credits.costs.CHAT_MESSAGE} crédito</span>
            </div>
          </div>
        </section>
      </div>
